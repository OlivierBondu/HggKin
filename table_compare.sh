...
for window in `echo "0 2 3"`
  ...
  if [[ ${window} == "0" ]] 
      ...
      echo "\\addtocontents{toc}{\\protect\\setcounter{tocdepth}{1}}"
  else
      echo "\\clearpage" 
      echo "\\addtocontents{toc}{\\protect\\setcounter{tocdepth}{2}}"
...
echo "\\section{\$p_T\$ Inclusive on \$|cos(\\theta*]|\$.}"
for categ in `seq 0 4`
  do
  if [[ ${categ} = "0" ]]
      then buffcateg=""
      echo "\\addtocontents{toc}{\\protect\\setcounter{tocdepth}{2}}"
      echo "\\subsection{Inclusive on Categories}"
      echo "\\addtocontents{toc}{\\protect\\setcounter{tocdepth}{1}}"
  else 
      echo "\\clearpage"
