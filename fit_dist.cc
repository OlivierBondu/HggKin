...
int main() {
   setTDRStyle();
  //#########MENU
   int const menu_bkg=0;//0:land; 1:logn; 2:tsallis 
   ...
   int const menu_vbf_pol=0; // degree of polynome
   char const *menu_cut=""; //"", cuttheta{200,375,550,750}
  ...
  RooRealVar roowindow("roowindow","roowindow",window);  
  TLatex latex;
  latex.SetNDC();
  ...
  sprintf(buffer,"dipho_mass<%2.2f && dipho_mass>%2.2f",125.+window/2.,125.-window/2.);
  cout << buffer << endl;

  if (window) dataset_bkg=new RooDataSet("dataset_bkg","dataset_bkg",tree_bkg,RooArgSet(dipho_pt,dipho_mass),buffer);
  ...
  switch (3*menu_bkg_pol+menu_bkg) {
  case 0 : 
    ...
    if (window)    sprintf(buffer_savebkg,"bkg%dlandpol0",window);
  ...
  case 3 : 
    ...
    model_bkg =new RooProdPdf("model_bkg","model_bkg",RooArgList(*land_bkg,*pol2_bkg));
    if (window)    sprintf(buffer_savebkg,"bkg%dlandpol1",window);
  ...
  case 4 : 
    ...
    if (window)    sprintf(buffer_savebkg,"bkg%dlognpol1",window);
    else sprintf(buffer_savebkg,"bkglognpol1");
    break;  
   case 5 :
    pol2_bkg=new RooPolynomial("pol2_bkg","pol2_bkg",dipho_pt,RooArgList(coef0_pol_bkg,coef1_pol_bkg));
    model_bkg =new RooProdPdf("model_bkg","model_bkg",RooArgList(*tsallis_bkg,*pol2_bkg));
    if (window)     sprintf(buffer_savebkg,"bkg%dtsallispol1",window);
  ...
  case 6 : 
    ...
    model_bkg =new RooProdPdf("model_bkg","model_bkg",RooArgList(*land_bkg,*pol2_bkg));
    if (window)     sprintf(buffer_savebkg,"bkg%dlandpol2",window);
    else     sprintf(buffer_savebkg,"bkglandpol2");
    break;  
  case 7 : 
    pol2_bkg=new RooPolynomial("pol2_bkg","pol2_bkg",dipho_pt,RooArgList(coef0_pol_bkg,coef1_pol_bkg,coef2_pol_bkg));
    model_bkg =new RooProdPdf("model_bkg","model_bkg",RooArgList(*logn_bkg,*pol2_bkg));
    if (window)     sprintf(buffer_savebkg,"bkg%dlognpol2",window);
    else     sprintf(buffer_savebkg,"bkglognpol2");
    break;  
 case 8 :
   pol2_bkg=new RooPolynomial("pol2_bkg","pol2_bkg",dipho_pt,RooArgList(coef0_pol_bkg,coef1_pol_bkg,coef2_pol_bkg));
    model_bkg =new RooProdPdf("model_bkg","model_bkg",RooArgList(*tsallis_bkg,*pol2_bkg));
    if (window)     sprintf(buffer_savebkg,"bkg%dtsallispol2",window);
...
pad_fit_bkg->cd();
  ...
  cout << buffer_savebkg << " : chi2 = " << frame_bkg->chiSquare() << endl;  
  ...
  RooRealVar coef2_logn_ggh("coef2_logn_ggh","coef2_logn_ggh",0,-60,30);
  ...
  switch (3*menu_ggh_pol+menu_ggh) {
  case 0 : 
 ...
 case 8 :
  ...
  cout << buffer_saveggh << " : chi2 = " << frame_ggh->chiSquare() << endl;    
...
pad_fit_vbf->cd();
  ...
  cout << buffer_savevbf << " : chi2 = " << frame_vbf->chiSquare() << endl;  
...
/*
  ...
  RooPlot *frame_dataggh=dipho_pt.frame();
  frame_dataggh->GetXaxis()->SetTitle("");
  RooPlot *frame_datavbf=dipho_pt.frame();
...
model_data=new RooAddPdf("model_data","model_data",RooArgList(*model_ggh,*model_bkg),compo_sgn);
  ...
  hist_data->Sumw2();
  RooDataHist *data=new RooDataHist("data","data",dipho_pt,hist_data);
  model_data->fitTo(*data);

  TH1F *ratio_data=(TH1F*)model_data->createHistogram("ratio_data",dipho_pt,RooFit::Binning(hist_data->GetNbinsX(),0,200));
...
//--------------------------------------
  ...
  hist_data->Sumw2();
  hist_data->Add(hist_bkg);
  data=new RooDataHist("data","data",dipho_pt,hist_data);
  model_data->fitTo(*data);

  ratio_data=(TH1F*) model_data->createHistogram("ratio_data",dipho_pt,RooFit::Binning(hist_data->GetNbinsX(),0,200));
