...
int AddModel(RooWorkspace *ws, int  const &cut=0) {
  ...
  if (cut) {  //select data according cuts on theta
    ...
    sim_gen=new RooDataSet("sim_gen","sim_gen",tree , RooArgSet(*dipho_mass,*dipho_pt,*dipho_ctheta,*weight), buffer,"weight");
  }
  ...
  cout << "exp_ggh_yield  " << exp_ggh_yield->getVal() << endl;
...
int AddData(RooWorkspace* ws, int const &cut=0) {
  ...
  sprintf(buffer,"");
  if (cut) sprintf(buffer,"%s && dipho_ctheta > %1.3f",buffer,cut/1000.); // create cut string for data set
  RooDataSet *sim_gen_bkg=new RooDataSet("sim_gen_bkg","sim_gen_bkg",tree,RooArgSet(*dipho_mass,*dipho_pt,*dipho_ctheta,*weight),buffer, "weight"); //Put cut tree into the dataset with weight
  sim_gen_ggh->append(*sim_gen_bkg); 
  ...
  sim_gen_ggh->plotOn(framemass, Binning(160,100,180));
  framemass->Draw();
...
//   canvas->SaveAs("frame_pt_gen.pdf");  
  ...
  ws->import(*sim_gen_ggh,RooFit::Rename("sim_gen"));
  file_kin->Close();
  cout << "end add data" << endl;
  return 0;}
...
int DoSPlot(RooWorkspace* ws) {
  ...
  cout << "expected ggh yield" << endl;
  sprintf(buffer,"expected = %3.0f",exp_ggh_yield->getVal());
  latex.DrawLatex(0.5,0.76,buffer);
  cout << "latex drawn" << endl;
...
int MakePlot(RooWorkspace* ws, int const &cut=0) {
  ...
  cout << "plotted ggh reweighted" << endl;
