...
int main() {
  ...
  char buffer[100];
    ...
    sprintf(buffer,"%s_genggh",buffer);
...
int AddModel(RooWorkspace *ws, int const &categ=0) {
  ...
  RooGaussian *model_ggh=new RooGaussian("model_ggh","model1_mass_ggh",*dipho_mass,*mean1_mass_ggh,*sigma1_mass_ggh);
  ...
  model_ggh->fitTo(*sim_gen,Range(110,140));
...
//   coefT_tsallis_pt_bkg->setConstant(1);
  ...
  for (int file=0; file<3; file++ ) {
    sprintf(buffer,"%s%sSPlotInput_ctheta%s_genggh.%s",buffer_path,buffer_file[0][file],buffercateg,buffer_file[1][file]);
...
int MakePlot(RooWorkspace* ws, int const &categ=0) {
  ...
  for (int file=0; file<3; file++ ) {
    sprintf(buffer,"%s%ssplot_ctheta%s_gengghbkg.%s",buffer_path,buffer_file[0][file],buffercateg,buffer_file[1][file]);
    cout << buffer << endl;
    canvas->SaveAs(buffer);
  }
  ...
  for (int file=0; file<3; file++ ) {
    sprintf(buffer,"%s%ssplot_ctheta%s_genggh.%s",buffer_path,buffer_file[0][file],buffercateg,buffer_file[1][file]);
    cout << buffer << endl;
    canvas->SaveAs(buffer);
  }
