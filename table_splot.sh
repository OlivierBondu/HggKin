...
for categ in `seq -1 3`
  ...
  echo "\\centering"
  echo "\\begin{minipage}[c]{0.49\linewidth}"
  echo "\\includegraphics[width=8cm]{/afs/cern.ch/work/c/cgoudet/private/plot/pdf/splot_unweighted_gghbkg${buffcateg}_reco.pdf}"
  echo "\\caption{Entrance Fit of mass and \$p_T\$ for Reconstructed Backgound + Reconstructed ggH events.}" 
  echo "\\end{minipage}"
  ...
  echo "\\begin{minipage}[c]{0.49\linewidth}"
  echo "\\centering"
  echo "\\includegraphics[width=8cm]{/afs/cern.ch/work/c/cgoudet/private/plot/pdf/splot_unweighted_gghbkg${cut}${buffcateg}_reco.pdf}"
  echo "\\caption{Entrance Fit of mass and \$p_T\$ for Reconstructed Backgound + Reconstructed ggH events with \$cos(\\theta*)>0.${cut}\$.}" 
  echo "\\end{minipage}"
