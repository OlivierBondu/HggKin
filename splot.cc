...
int main() {
  ...
  //############def
  int AddModel(RooWorkspace*, int const &cut=0);
  int AddData(RooWorkspace*, int const &cut=0);
...
//     frame_mass->Delete();
  ...
  // RooGenericPdf *tsallis_pt_ggh=new RooGenericPdf("tsallis_pt_ggh","dipho_pt*pow(1+(sqrt(coefM_tsallis_pt_ggh*coefM_tsallis_pt_ggh+dipho_pt*dipho_pt)-coefM_tsallis_pt_ggh)/coefN_tsallis_pt_ggh/coefT_tsallis_pt_ggh,-coefN_tsallis_pt_ggh)",RooArgSet(*dipho_pt,*coefN_tsallis_pt_ggh,*coefM_tsallis_pt_ggh,*coefT_tsallis_pt_ggh));
...
int AddData(RooWorkspace* ws, int const &cut=0) {
  ...
  TTree *tree=(TTree*) file_kin->Get("tree_ggh");
    ...
    RooDataSet *sim_gen_ggh=new RooDataSet("sim_gen_ggh","sim_gen_ggh",tree,RooArgSet(*dipho_mass,*dipho_pt,*dipho_ctheta),buffer);
    cout << "ggh entries =" << tree->GetEntries() << endl;
  tree->Delete();
  ...
  cout << "bkg entries =" << tree->GetEntries() << endl;
  cout << "final dataset entries =" << sim_gen->numEntries() << endl;
...
//   canvas->Close();
  ...
  cout << "before splot" << endl;
  cout << sim_gen->numEntries() << endl;
  //  RooStats::SPlot *sdata=new RooStats::SPlot();  
  SPlot *splot_gen=new SPlot("splot_gen","splot_gen", *sim_gen, model, RooArgList(*ggh_yield, *bkg_yield));  

  cout << "Check the Yields" << endl;
  cout << "bkg yields=" << bkg_yield->getVal() << endl;
  cout << "bkg weighted yields=" << splot_gen->GetYieldFromSWeight("bkg_yield") << endl;
  cout << "dataset entries " << bkg_yield->getVal() << " " << ggh_yield->getVal() << endl; 
  return 0;
}
...
int MakePlot(RooWorkspace* ws, int const &cut=0) {
  ...
  sim_gen_Wbkg->plotOn(frame_up,MarkerColor(1),DataError(RooAbsData::SumW2),Name("sim_gen_Wbkg"));
  RooDataSet *sim_gen_Wggh=new RooDataSet("sim_gen_Wbkg","sim_gen_Wbkg",sim_gen,*sim_gen->get(),0,"ggh_yield_sw");
  sim_gen_Wggh->plotOn(frame_up,MarkerColor(2),DataError(RooAbsData::SumW2),Name("sim_gen_Wggh"));
  model->plotOn(frame_up,Components("model_bkg"),LineColor(4),Normalization(sim_gen_Wbkg->numEntries(),RooAbsReal::NumEvent),Name("model_bkg"));
  model->plotOn(frame_up,Components("model_ggh"),LineColor(3),Normalization(sim_gen_Wggh->numEntries(),RooAbsReal::NumEvent),Name("model_ggh"));
  cout << "before cd" << endl;
  canvas->cd();
  cout << "before draw" << endl;
  frame_up->UseCurrentStyle();
  frame_up->Draw();
  cout << "plotted weighted" << endl;
...
//   }
  ...
  cout << sim_gen_Wbkg->numEntries() << " " << sim_gen_Wggh->numEntries() << endl;
